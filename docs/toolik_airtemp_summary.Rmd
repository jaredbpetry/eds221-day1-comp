...
#----the here function always starts at the level of the working directory, which is the Rproject file... so all you have to say is, first look in data, then data raw, and then theres the csv folder in there
```
...
```{r}
#-----tidyverse pipe operator is %>% 
#-----this makes writing code feel more intuitive 
#-----when you write sequences as nested functions, you have to line up the parenthesis, looks like everything is happening backwards, and can quickly become really hard to understand... pipe operator lets us do the exact same thing but write the steps in the order that they actually happen YAY
...
#---new use of the here:here function... saying where to save something rather than where to pull something from 
